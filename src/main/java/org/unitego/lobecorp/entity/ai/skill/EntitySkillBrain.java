...
public class EntitySkillBrain {
    /// 当前激活技能的状态（运行时，不持久化）
    public static final MemoryModuleType<EntitySkillRuntime> SKILL_ACTIVE = LcMemoryModuleTypes.SKILL_ACTIVE.get();
    /// 技能冷却表：skillId → 冷却结束的世界 tick（运行时，不持久化）
    public static final MemoryModuleType<Map<Identifier, Long>> SKILL_COOLDOWNS = LcMemoryModuleTypes.SKILL_COOLDOWNS.get();
    ...
    public static boolean isOnCooldown(Mob entity, IEntitySkill skill) {
        Map<Identifier, Long> cooldowns = entity.getBrain().getMemory(SKILL_COOLDOWNS).orElse(Map.of());
        Long until = cooldowns.get(skill.id());
        return until != null && until > entity.level().getGameTime();
    }

    /// 是否满足施放条件（冷却 + 未占用 + 技能自身条件）
    public static boolean canCast(Mob entity, IEntitySkill skill) {
        if (isOnCooldown(entity, skill)) {
            return false;
        }
        if (entity.getBrain().getMemory(SKILL_ACTIVE).isPresent()) {
            return false;
        }
    ...
    public static boolean cast(Mob entity, IEntitySkill skill) {
        if (!canCast(entity, skill)) {
            return false;
        }
        EntitySkillRuntime runtime = new EntitySkillRuntime(skill, EntitySkillRuntime.SkillState.WINDUP, skill.windupTicks());
    ...
    public static void endSkill(Mob entity) {
        ...
        EntitySkillRuntime runtime = brain.getMemory(SKILL_ACTIVE).orElse(null);
    ...
    public static void tick(ServerLevel level, Mob entity, EntitySkillRuntime runtime) {
        IEntitySkill skill = runtime.skill();
    ...
    private static void enterRecovery(EntitySkillRuntime runtime, IEntitySkill skill) {
        runtime.setState(EntitySkillRuntime.SkillState.RECOVERY);
    ...
    public static void setCooldown(Mob entity, IEntitySkill skill, int ticks) {
        ...
        Brain<?> brain = entity.getBrain();
        Map<Identifier, Long> cooldowns = new HashMap<>(brain.getMemory(SKILL_COOLDOWNS).orElse(Map.of()));
